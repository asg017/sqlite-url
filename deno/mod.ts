...
let path: string;
try {
  const customPath = Deno.env.get("DENO_SQLITE_URL_PATH");
  if (customPath) path = customPath;
  else {
    path = await download({
      url: {
        darwin: {
          x86_64: `${BASE}/deno-darwin-x86_64.url0.dylib`,
        },
        windows: {
          x86_64: `${BASE}/deno-windows-x86_64.url0.dll`,
        },
        linux: {
          x86_64: `${BASE}/deno-linux-x86_64.url0.so`,
        },
      },
      suffixes: {
        darwin: "",
  ...
  const error = new Error("Failed to load sqlite-url extension");
  error.cause = e;
...
/**
 * Returns the full path to the compiled sqlite-url extension.
 * Caution: this will not be named "url0.dylib|so|dll", since plug will
...
/**
 * Entrypoint name for the sqlite-url extension.
 */
export const entrypoint = "sqlite3_url_init";
...
/**
 * Loads the sqlite-url extension on the given sqlite3 database.
 */
